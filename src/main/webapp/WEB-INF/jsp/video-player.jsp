									...
									<ul class="main-info-list">
										...
										<li class="duration">
											<div class="ico-wrapper">
												<a id="add-to-fav" href="javascript:;">
													<svg version="1.1" id="fav" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
														...
														<path fill="#FFFFFF" d="M12.645,18.485L20.126,23l-1.985-8.51l6.609-5.726l-8.704-0.738L12.645,0L9.243,8.026L0.539,8.764
...
$("#add-to-fav").click(function(e){
    ...
    $.ajax({url: "../addtofav/${video.id}", success: function(result){
        $("#add-to-fav").addClass('fav-red');
    }});
});

var voteDown = function (){
    var currDown = $(".vote-down a").parent().next().html();
    console.log('vote down');
    console.log(currDown);

    $.ajax({url: "../voteDown/${video.id}", success: function(result){
        $(".vote-down a").parent().next().html(parseInt(currDown) - 1);
        $(".vote-down a").off("click");
        $(".vote-up a").on('click', voteUp)
        $(".vote-up a").removeClass('svgRed')
        $('.vote-down .ico-wrapper a').addClass('svgRed');
    }});
}

var voteUp = function(){
    var currUp = $(".vote-up a").parent().next().html();
    console.log('vote up');
    console.log(currUp);

    $.ajax({url: "../voteUp/${video.id}", success: function(result){
        $('.vote-up a').parent().next().html(parseInt(currUp) + 1);
        $(".vote-up a").off("click");
        $(".vote-down a").on('click', voteDown)
        $(".vote-down a").removeClass('svgRed')
        $('.vote-up .ico-wrapper a').addClass('svgRed');
    }});

}

$(".vote-down a").click(function(e){
    e.preventDefault();
    voteDown();
});
